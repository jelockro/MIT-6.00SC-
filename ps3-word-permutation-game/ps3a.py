...
def get_word_score(word, hand_size):
    ...
    for letter in word:
        letterScoreList.append(SCRABBLE_LETTER_VALUES[letter])
    wordScore = sum(letterScoreList)*len(word)
...
def is_valid_word(word, hand, word_list):
    ...
    if word in word_list:
        ...
        testHand=dict(hand)
        for i in word:
            if testHand.get(i, False) >= 1:
                testString = testString + str (i)
                testHand[i]=hand.get(i,0) -1
            else:
                return False
        if testString == word:
            return True
...
def play_game(word_list):
    ...
    * If the user inputs anything else, ask them again.
    ...
    lastHand = {}
    while True:
        status = raw_input("input 'n', for new hand; 'r' for replay of last hand; or 'e' to exit the game....:")
        if status == 'e':
            return False
        elif status == 'n':
